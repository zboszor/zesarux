...
void debug_get_ioports(char *stats_buffer)
{
  	...
  	if (MACHINE_IS_SPECTRUM_P2A || MACHINE_IS_ZXUNO_BOOTM_DISABLED || MACHINE_IS_PRISM || superupgrade_enabled.v || MACHINE_IS_CHROME || TBBLUE_MACHINE_P2A) {
...
void to_delete_debug_get_memory_pages(char *texto_final)
{
  ...
  //De momento por si acaso, cadena vacia
  texto_memoria[0]=0;
  			...
  			if (MACHINE_IS_SPECTRUM_16_48 && multiface_enabled.v && multiface_switched_on.v) {
  				debug_registers_get_mem_page(0,texto_memoria);
                          ...
                          if (MACHINE_IS_ZXUNO ) {
                                  ...
                                  for (pagina=0;pagina<4;pagina++) {
                                          debug_registers_get_mem_page_zxuno(pagina,texto_paginas[pagina]);
                                  }
  																if (ZXUNO_BOOTM_ENABLED) sprintf (texto_paginas[0],"%s","RO");

                                  //sprintf (texto_memoria,"%s %s %s %s",texto_paginas[0],texto_paginas[1],texto_paginas[2],texto_paginas[3]);
  																sprintf (texto_memoria,"%s %s %s %s SCR%d %s",texto_paginas[0],texto_paginas[1],texto_paginas[2],texto_paginas[3],
  																																( (puerto_32765&8) ? 7 : 5) ,  ( (puerto_32765&32) ? "PDI" : "PEN"  ) );
  ...
  //D5
  			...
  			//BANK PAGES
  			if (MACHINE_IS_Z88) {
  				sprintf (texto_memoria,"BANK%02X BANK%02X BANK%02X BANK%02X",blink_mapped_memory_banks[0],blink_mapped_memory_banks[1],
...
//Paginas memoria
				...
				if (ZXUNO_BOOTM_ENABLED) {
					...
					sprintf (segmentos[0].longname,"%s","ROM");
				}
